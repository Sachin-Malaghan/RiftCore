 ...
 * 
 * This is the PRIMARY UI system for the RiftCore Editor. It consolidates
 * all UI functionality including docking, menu bar, toolbar, and panel management.
 * 
...
bool HUD::Initialize(GLFWwindow* window, const HUDConfig& config) {
    ...
    if (!config.FontPath.empty()) {
        m_DefaultFont = io.Fonts->AddFontFromFileTTF(
            config.FontPath.c_str(), config.FontSize);
    }
    if (!m_DefaultFont) {
...
void HUD::ApplyDarkTheme() {
    ...
    // Custom RiftCore dark color scheme
...
void HUD::LoadDefaultIcons() {
    // Try to load standard editor icons
...
unsigned int HUD::LoadIcon(const std::string& path) {
    ...
    if (!data) {
        // Silently fail for missing icons - not critical
        return 0;
    }

    // Invert colors for dark theme icons (makes them white on dark background)
...
void HUD::OnUIRender() {
    ...
    // Render the main docking workspace
    RenderMainDockspace();
}
...
void HUD::RenderMainDockspace() {
    // Get viewport and set up full-screen dockspace
...
void HUD::SetupDockspace() {
    ...
    // Split: Left panel (Hierarchy) | Center (Viewport) | Right (Inspector)
    ...
    // Split bottom from center for Console/Asset Browser
    ImGuiID dockBottom;
    ...
    if (ImGui::BeginMenuBar()) {
        ...
        if (ImGui::BeginMenu("File")) {
            ...
            if (ImGui::MenuItem("Save Scene As...", "Ctrl+Shift+S")) {
                m_CommandBuffer.Push(EditorCommandType::SaveSceneAs);
                if (m_Callbacks.OnSaveSceneAs) m_Callbacks.OnSaveSceneAs();
            }
            ImGui::Separator();
            if (ImGui::MenuItem("Import Asset...")) {
                m_CommandBuffer.Push(EditorCommandType::ImportAsset);
            }
            ImGui::Separator();
        ...
        if (ImGui::BeginMenu("Edit")) {
            ...
            if (ImGui::MenuItem("Cut", "Ctrl+X")) {
                m_CommandBuffer.Push(EditorCommandType::Cut);
            }
            if (ImGui::MenuItem("Copy", "Ctrl+C")) {
                m_CommandBuffer.Push(EditorCommandType::Copy);
            }
            if (ImGui::MenuItem("Paste", "Ctrl+V")) {
                m_CommandBuffer.Push(EditorCommandType::Paste);
            }
            if (ImGui::MenuItem("Delete", "Del")) {
                m_CommandBuffer.Push(EditorCommandType::Delete);
            }
            ImGui::Separator();
            if (ImGui::MenuItem("Duplicate", "Ctrl+D")) {
                m_CommandBuffer.Push(EditorCommandType::Duplicate);
            }
            if (ImGui::MenuItem("Select All", "Ctrl+A")) {
                m_CommandBuffer.Push(EditorCommandType::SelectAll);
            }
            ImGui::Separator();
            if (ImGui::MenuItem("Preferences...")) {
                m_CommandBuffer.Push(EditorCommandType::Preferences);
            }
            ImGui::EndMenu();
        }
        ...
        if (ImGui::BeginMenu("View")) {
            if (ImGui::MenuItem("Toggle Grid", "G")) {
                m_CommandBuffer.Push(EditorCommandType::ToggleGrid);
            }
            if (ImGui::MenuItem("Toggle Wireframe", "Z")) {
                m_CommandBuffer.Push(EditorCommandType::ToggleWireframe);
            }
            if (ImGui::MenuItem("Toggle Stats", "F3")) {
                m_ShowStats = !m_ShowStats;
                m_CommandBuffer.Push(EditorCommandType::ToggleStats);
            }
            ImGui::Separator();
            if (ImGui::MenuItem("Focus Selected", "F")) {
                m_CommandBuffer.Push(EditorCommandType::FocusSelected);
            }
            ImGui::EndMenu();
        }
        ...
        if (ImGui::BeginMenu("Window")) {
            ...
            if (ImGui::MenuItem("Reset Layout")) {
                m_FirstFrame = true; // Will trigger SetupDockspace
            }
            ImGui::EndMenu();
        }
        ...
        if (ImGui::BeginMenu("Help")) {
            ...
            if (ImGui::MenuItem("About RiftCore")) {
...
void HUD::RenderToolbar() {
    ...
    // Undo/Redo
    if (IconButton("Undo", "Undo")) {
        m_CommandBuffer.Push(EditorCommandType::Undo);
        if (m_Callbacks.OnUndo) m_Callbacks.OnUndo();
    }
    ImGui::SameLine();
    if (IconButton("Redo", "Redo")) {
        m_CommandBuffer.Push(EditorCommandType::Redo);
        if (m_Callbacks.OnRedo) m_Callbacks.OnRedo();
    }
    ...
    // Settings
    if (IconButton("Settings", "Settings")) {
        m_CommandBuffer.Push(EditorCommandType::Settings);
    }

    ImGui::EndGroup();

    // ── Play Controls (Center-Right) ────────────────────────
...
bool HUD::IconButton(const char* label, const char* iconKey, float size) {
    ...
    if (texID != 0) {
        ...
        // Fallback to text button if no icon
...
//=============================================================================
// LEGACY RENDER MODE (for compatibility with old code)
//=============================================================================
...
void HUD::DrawSelectedObjectPanel(
    ...
    if (selectedIdx < 0 || selectedIdx >= static_cast<i32>(objects.size()))
        return;
...
void HUD::DrawObjectListPanel(
    ...
    if (ImGui::Begin("Scene Objects", nullptr, flags)) {
        for (i32 i = 0; i < static_cast<i32>(objects.size()); i++) {
            ...
            ImGui::SameLine(200);
            ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.6f, 1.0f), "(%.1f,%.1f,%.1f)",
...
void HUD::DrawStatsPanel(const HUDRenderStats& stats) {
    ...
    // Empty implementation - stats shown in main panel
}

void HUD::DrawCameraPanel(const HUDCameraInfo& cam) {
    RIFTCORE_UNUSED(cam);
    // Empty implementation - camera info shown in main panel
}
